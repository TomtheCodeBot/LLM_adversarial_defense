...
class AttackPrompt(object):
        ...
        if return_ids:
            ...
            for i in range(iter_batches):
                ...
                if attn_mask is not None:
                    attn_mask_iter = attn_mask[i*interation_batch_size:(i+1)*interation_batch_size]
                else:
    ...
    def target_loss(self, logits, ids):
        logits = logits.requires_grad_(False)
        ids = ids.requires_grad_(False)
        ...
        for i in range(n_steps):
            ...
            if stop_on_success:
                ...
                if all(all(tests for tests in model_test) for model_test in model_tests_jb):
                    model_tests = self.test_all()
                    print(model_tests)
                    self.log(i+1+anneal_from, n_steps+anneal_from, self.prompts[0].control_str, best_loss, runtime, model_tests, verbose=verbose)
                    break
