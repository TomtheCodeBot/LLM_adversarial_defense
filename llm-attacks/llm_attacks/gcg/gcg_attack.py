        ...
        with torch.no_grad():
            for j, cand in enumerate(control_cands):
                ...
                for i in progress:
                    ...
                    #for k, (logit, id) in enumerate(zip(logits, ids)):
                    #    mold = None
                    #    iter_batches = len(logit) // interation_batch_size
                    #    iter_batches +=1 if len(logit) % interation_batch_size !=0 else 0
                    #    for iter in range(iter_batches):
                    #        logits_iter = logit[iter*interation_batch_size:(iter+1)*interation_batch_size]
                    #        ids_iter = id[iter*interation_batch_size:(iter+1)*interation_batch_size]
                    #        if mold is None:
                    #            mold = target_weight*self.prompts[k][i].target_loss(logits_iter, ids_iter).to(main_device) 
                    #        else:
                    #            mold = torch.cat([mold,target_weight*self.prompts[k][i].target_loss(logits_iter, ids_iter).to(main_device)],0)
                    #    mold = mold.mean(dim=-1)
                    #    loss+=mold
                    if control_weight != 0:
                        loss[j*batch_size:(j+1)*batch_size] += sum([
